...
chcp 65001 >nul
echo =====================================================
echo   NMGone Installer Build V2.2.0 (inkl. NMG Kasse)
echo =====================================================
echo.
...
if exist "%ISCC%" (
  "%ISCC%" installer\NMGone_Setup_2_2_0.iss
  if errorlevel 1 (
  ...
  echo.
  echo Setup-Datei: dist_setup\NMGone_Setup_2_2_0.exe
) else (
  echo Inno Setup 6 nicht gefunden. Installiere via: winget install JRSoftware.InnoSetup
  echo Skript-Vorlage liegt unter installer\NMGone_Setup_2_2_0.iss
)
pause

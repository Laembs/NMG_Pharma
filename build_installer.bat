...
chcp 65001 >nul
echo =====================================================
echo   NMGone Installer Build V1.1 SP3 (1.1.3)
echo =====================================================
echo.
...
if exist "%ISCC%" (
  "%ISCC%" installer\NMGone_Setup_1_1_3.iss
  if errorlevel 1 (
  ...
  echo.
  echo Setup-Datei: dist_setup\NMGone_Setup_1_1_3.exe
) else (
  echo Inno Setup 6 nicht gefunden. Installiere via: winget install JRSoftware.InnoSetup
  echo Skript-Vorlage liegt unter installer\NMGone_Setup_1_1_3.iss
)
pause
